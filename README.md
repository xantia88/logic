...
1. есть две базы знаний, и у каждой есть свой язык (пусть одна будет про микросервисы, а другая про техническую архитектуру)
2. система распознавания речи разбивает речь пользователя на слова и оценивает насколько набор произнесенных слов близок к языку микросервисов или к языку технической архитектуры
3. система подключает ту базу знаний к которой речь пользователя ближе и запускает интерактивные диалоги для уточнения деталей, сбора дополнительных знаний и ответа на пользоваетльские вопросы
4. у пользователя складывается впечатление, что система понимает контекст
