...
Например, пусть у нас есть 2 системы - одна называется Аналитика, другая - Бухгалтерия. Пусть для каждой из систем заданы уровни критичности - для одной Critical, для другой - Productivity. И пусть для одной из систем указано RTO 24, а для другой 100. Опишем это как факты на  Prolog.
...
Это данные, которые читаются на русском языке. Prolog позволяет придумывать любые структуры и описывать их как удобно пользователю, что бы получилась удобная читаемость. Данные читаются, их можно понять. Почему это код ? Эти данные можно выполнить и получить полезный для себя ответ. Например, я хочу задать вопрос - какая система очень критична, но имеет RTO больше 25? Напишем этот запрос к своим данным используя свои же придуманные структуры и даже на русском языке.
