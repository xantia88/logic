...
Для оценки произволительности в базу знаний добавлялись интеграционные потоки, связанные с двугими сущностями (системами, объектами данных). Для валидации использовалась valid(integration(X)) последовательно для каждого потока. Для анализа использовалась explain(integration(X)) последовательно для каждого потока.
