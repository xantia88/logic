...
Prolog - технология логического программирования, позволяющая работать с базами знаний, делать к ним запросы, обогащать данными, проверять на корректность и непротиворечивость, делать сложные реурсивные логические выводы. 
...
|-----|------|------|------|
|1|Разработка логической модели архитектуры дочернего общества|Создание единого языка, структур и правил работы с ними|Логическая модель на языке Prolog|
|2|Описание архитектуры дочернего общества ввиде базы знаний|Цифровизация знаний, возможность делать логические выводы на их основе|База знаний на языке Prolog|
...
|3|Реализация функциональности сравнения двух разных версий архитектурных моделей|Найти что изменилось и проанализировать разницу между двумя состояниями базы знаний (двумя версиями архитектуры)|Сравнить все факты и отношения в двух версиях баз знаний|
|4|Экспорт описания архитектуры дочернего общества в форматах YAML (DocHub), SQL, XML (ArchiMate)|Возможность работы с представлением знаний в сторонних инструментах|Преобразовать базу знаний в набор текстовых файлов|
|5|Автоматическое формирование архитектурных решений и рекомендаций с использований логических моделей, разработанных родительской компанией или дочерней компанией|Применение лучших практик, шаблонов проектирования, проверка стандартов|Описание "что правильно" ввиде логической модели на языке Prolog, с целью применить эту модель к базе знаний и получить ответы - "что неправильно" и "как сделать правильно"|
...
Для оценки произволительности в базу знаний добавлялись интеграционные потоки, связанные с двугими сущностями (системами, объектами данных). Для валидации использовалась цель **valid(integration(X))** последовательно для каждого интеграционного потока. Для анализа использовалась цель **explain(integration(X))** последовательно для каждого интеграционного потока.
