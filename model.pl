...
valid(process(X)):-
    ...
    (   % проверка обеспечивающих систем
        ...
        findall(S, systems(X, S), LS), forall(member(S, LS), valid(system(S))), ! % выбрать системы и проверить, что они валидны
        ; fail % иначе завершить проверку с ошибкой
    ),
    (   % проверка используемых продуктов
        use(X, P), product(P) -> % если бизнес процесс использует продукты
        findall(P, products(X, P), LP), forall(member(P, LP), valid(product(P))), ! % выбрать продукты и проверить, что они валидны
        ; fail % иначе завершить проверку с ошибкой
    ),
    (
        % проверка используемых интеграционных потоков
        use(X, I), integration(I) -> % если бизнес процесс использует интеграции
        findall(I, integrations(X, I), LI), forall(member(I, LI), valid(integration(I))), ! % выбрать интеграции и проверить, что они валидны
        ; fail % иначе завершить проверку с ошибкой
    ), 
    (
        % проверка реаизуемых бизнес объектов
        use(X, O), object(O) -> % если бизнес процесс использует бизнес объекты
        findall(O, objects(X, O), LO), forall(member(O, LO), valid(object(O))), ! % выбрать бизнес объекты и проверить, что они валидны
        ; fail % иначе завершить проверку с ошибкой
    ).

% проверка валидности бизнес объекта (Р07)
valid(object(X)):-
    object(X), % должен быть определен как объект данных
    name(X, _), % должно быть задано наименование
...
valid(data(X)):-
    ...
    status(X, Y), change_type(_, Y), % должен быть указан статус изменений из соотвествующего справочника
    system(X, S), valid(system(S)), % должна быть указана валидная мастер система
    parent(X, B), valid(object(B)). % должен быть указан валидный родительский бизнес объект  

% проверка валидности интеграционного потока (Р14)
valid(integration(X)):-
    integration(X), % должен быть определен как интеграционный поток
    description(X, _), % должно быть задано описание
    source(X, From), valid(system(From)), % должна быть указана система источник данных
    destination(X, To), valid(system(To)), % должна быть указана система потребитель данных
    ...
    load(X, max(_), avg(_)), % должны быть указаны максимальные и средние нагрузочные характеристики
...
valid(component(X)):-
    ...
    system(X, S), valid(system(S)), % должена быть задана валидная система
...
explain(integration(X)):-
    ...
    (
        ...
        (   % анализ параметров нагрузки
            load(X, max(_), avg(_)) -> % если параметры нагрузки заданы
...
explain(process(X)):-
    ...
    (
        ...
        (   % анализ обеспечивающих систем
            ...
            findall(S, systems(X, S), LS), forall(member(S, LS), explain(system(S))), ! % выбрать системы и перейти к их анализу
        ...
        (   % анализ используемых продуктов
            use(X, P), product(P) -> % если бизнес процесс использует продукты
            findall(P, products(X, P), LP), forall(member(P, LP), explain(product(P))), ! % выбрать продукты и перейти к их анализу
        ...
        (   % анализ используемых интеграционных потоков
            ...
            findall(I, integrations(X, I), LI), forall(member(I, LI), explain(integration(I))), ! % найти интеграционные потоки и перейти к их анализу
        ...
        (   % анализ используемых бизнес объектов
            use(X, O), object(O) -> % если бизнес процесс использует бизнес объекты
            findall(O, objects(X, O), LO), forall(member(O, LO), explai(object(O))), ! % найти бизнес объекты и перейти к их анализу
