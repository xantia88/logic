...
:-discontiguous(business_object/1). % бизнес объект
...
% проверка корректности бизнес объекта (Р07)
valid_business_object(X):-
    business_object(X), % должен быть задан как объект данных
    name(X, _), % должно быть задано наименование
...
valid_data_object(X):-
    ...
    parent(X, B), valid_business_object(B). % должен быть указан валидный родительский бизнес объект  
...
% проверка корректности технического компонента (Р16)
valid_component(X):-
...
explain_business_object(X):-
    write("Бизнес объект "), write(X), write(" "),
    (
        business_object(X) -> % если бизнес объект определен как сущность
        write(" определен"), nl, 
        validate(name, X), % проверить, что задано наименование
...
explain_data_object(X):-
    ...
    (
        ...
        (
            ...
            explain_business_object(B), ! % перейти к анализу родительского бизнес объекта
            ; write("Ошибка: должен быть указан родительский бизнес объект") % иначе сообщить об ошибке
        ), ! % завершить анализ
        ; write(" не определен"), nl % иначе сообщить об ошибке
    ).
...
% анализ технического компоненты (Р16)
explain_component(X):- 
