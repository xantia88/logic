...
start:-
    ...
    dialog(it), % выяснить подключена ли система к централизованным ИТ сервисам
...
dialog(roles):-
    ask("В системе применяется ролевая модель для пользователей", Roles),
    (  
        true(Roles) ->
        ask("Управление ролевой моделью пользователей в системе централизовано на уровне компании"), !
    ;   ask("В системе применяется управление полномочиями для пользователей", Privileges),
        ( 
            true(Privileges) ->
            ask("Управление полномочиями пользователей в системе централизовано на уровне компании")
        ;   !
        )
    ).
